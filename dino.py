...
class T_Rex():
    def __init__(self, option = 3):
        ...
        self.input_handler = InputHandlerPipe()
    ...
    def update(self):
        ...
        if not self.jumping:
            if self.input_handler.is_jumping():
                self.jumping = True
                self.speed = SPEED_TREX
            elif self.input_handler.is_ducking():
                self.lowering = True
            ...
            if self.y <= Y_TREX - HIGH_MIN and self.speed < 0 and (not self.input_handler.is_jumping()):
                self.speed = 0
...
class ListCatusAndBirds():
    ...
    def update(self, score):
        ...
        if self.list[0].x < -132:
            self.list.pop(0)
            if self.speed > SPEED_GROUND * 1.1:
