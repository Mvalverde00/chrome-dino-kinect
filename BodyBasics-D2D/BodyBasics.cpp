...
LPCSTR pipename = "\\\\.\\pipe\\joint_states";
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            LPTSTR lpvMessage = (LPTSTR)(temp.c_str());
                            DWORD cbToWrite = (lstrlen(lpvMessage) + 0) * sizeof(TCHAR);
                            DWORD cbWritten;
                            ...
                            if (!fSuccess) {
                              ...
                              if (err == ERROR_NO_DATA) { // client closed connection
                                std::cout << "Client closed pipe, resetting pipe for writing.\n";
